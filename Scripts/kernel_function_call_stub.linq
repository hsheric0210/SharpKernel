...
// T1: stacksize 0x58
var t1 = new Type[] { typeof(byte), typeof(short), typeof(int), typeof(long), typeof(void*), typeof(byte), typeof(short), typeof(int), typeof(long), typeof(void*) };

// T2: stacksize 0x38
var t2 = new Type[] { typeof(void*), typeof(long), typeof(int), typeof(short), typeof(byte) };

// T3: stacksize 0x28
var t3 = new Type[] { typeof(long), typeof(void*), typeof(byte), typeof(short) };

var shellCode = new KernelFunctionCallStub(functionAddress, t1, 16).GetShellcode().Select(t=>$"{t:X2}").ToArray();
Console.WriteLine(string.Join("", shellCode.Select(b=>$"\\x{b}")));

shellCode = new KernelFunctionCallStub(functionAddress, t2, 16).GetShellcode().Select(t=>$"{t:X2}").ToArray();
Console.WriteLine(string.Join("", shellCode.Select(b=>$"\\x{b}")));

shellCode = new KernelFunctionCallStub(functionAddress, t3, 16).GetShellcode().Select(t=>$"{t:X2}").ToArray();
Console.WriteLine(string.Join("", shellCode.Select(b=>$"\\x{b}")));
